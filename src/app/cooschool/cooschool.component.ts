...
@Component({
  selector: 'app-cooschool',
  templateUrl: './cooschool.component.html',
  styleUrls: ['./cooschool.component.scss']
})
export class CooschoolComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.http.get<{students: Student[]}>('http://localhost:3000/students').subscribe((Data) => {
      ...
      for(let h of this.students){
          const studentId = h._id ? h._id.toString() : "";
          const matchesSelected = this.studentID && studentId === this.studentID.toString();
          const matchesFullNameParam = this.fullName && studentId === this.fullName.toString();
          if(matchesSelected || matchesFullNameParam){
            this.student = h;
            this.studentID = h._id;
            this.fullName = (h.firstName || "") + " " + (h.lastName || "");
