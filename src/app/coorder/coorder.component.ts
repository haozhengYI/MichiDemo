...
@Component({
  selector: 'app-coorder',
  templateUrl: './coorder.component.html',
  styleUrls: ['./coorder.component.scss']
})
export class CoorderComponent implements OnInit {
      ...
      this.http.get<{students: Student[]}>('http://localhost:3000/students/').subscribe((Data) => {
          ...
          this.http.get<{schools: School[]}>('http://localhost:3000/schools').subscribe((schoolData) => {
              const studentIds = new Set(
                this.students.filter(st => st._id).map(st => st._id.toString())
              );
              this.schools = (schoolData.schools || []).filter(sc =>
                sc.userAccount && studentIds.has(sc.userAccount.toString())
              );
              this.school = [...this.schools];
              console.log('当前协调员申请数据:', this.schools);
  ...
  initializeYears() {
    this.uniqueYears = Array.from(new Set(
      this.students
        .filter(student => student && student.year)
        .map(student => student.year.toString())
    ));

    this.uniqueYears.sort((a, b) => {
      const aNum = /^\d{4}/.test(a);
      const bNum = /^\d{4}/.test(b);
      if (aNum && bNum) {
    ...
    if (this.uniqueYears.length > 0) {
      const yearWithApps = this.uniqueYears.find(year => this.getYearApplicationCount(year) > 0);
      this.selectedYear = yearWithApps || this.uniqueYears[0];
    }
  ...
  getYearApplicationCount(year: string): number {
    ...
    return this.school.filter(app => {
      // 通过ID匹配学生
      const student = this.students.find(st => st._id && app.userAccount && st._id.toString() === app.userAccount.toString());
      return student && student.year && student.year.toString() === year;
    }).length;
  }
  ...
  getCurrentYearApplications(): School[] {
    ...
    return this.school.filter(app => {
      // 通过ID匹配学生
      const student = this.students.find(st => st._id && app.userAccount && st._id.toString() === app.userAccount.toString());
      return student && student.year && student.year.toString() === this.selectedYear;
    });
  }

  // 获取当前学年的所有学生
  getCurrentYearStudents(): Student[] {
    return this.students.filter(student => student.year && student.year.toString() === this.selectedYear);
  }
  ...
  hmschooldetail(s) {
    const navigationExtras: NavigationExtras = {
      queryParams: {
       ...
       "fullName" : this.getStudentName(s.userAccount),
       "schoolID"  : s._id,
      }
    };
    this.router.navigate(['/cooschool'], navigationExtras);
  }
