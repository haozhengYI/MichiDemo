...
@Component({
  selector: 'app-studentaddinte',
  templateUrl: './studentaddinte.component.html',
  styleUrls: ['./studentaddinte.component.scss']
})
export class StudentaddinteComponent implements OnInit {
  students: Student[] = [];
  student : Student;
  ...
  constructor(
    ...
    addinterview(form: NgForm){//form内的信息使用过html中的name来获取的
      ...
      this.interviewService.addInterview("", form.value.interviewUniv,form.value.interviewSchool,form.value.interviewProgram,form.value.interviewTitle, form.value.interviewCato, currentDate, this.student.firstName + " " + this.student.lastName, form.value.interviewDuration, form.value.interviewOther,
     );
