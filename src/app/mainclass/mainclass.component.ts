...
export class MainclassComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  checklogin(){
    ...
    let temp9 = ((document.getElementById("login-user-account") as HTMLInputElement).value || "").trim();
    ...
    // Always fetch users first, then validate — avoids racing ngOnInit's async load
    // (which previously caused "No such account" even when the account existed).
    this.http.get<{user: user[]}>('http://localhost:3000/users').subscribe((userData) => {
      this.user = userData.user;
      this.validateLogin(temp9, temp10);
    }, () => {
      alert("Unable to reach login service. Please try again.");
    });
  }

  private validateLogin(temp9: string, temp10: string) {
    ...
    for (let index = 0; index < this.user.length; index++) {
      ...
      alert("Unknown role: " + this.user[index].Role);
      return;
    }
